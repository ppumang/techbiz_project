...
def tfidf_counter(word_list, tf_min=3, df_max=-1):
    ...
    counter = counter.sort_values(
        'tf-idf', ascending=False).reset_index(drop=1)
        ...
        for idx, word in enumerate(text):
            ...
            if type(temp) == re.Match:
                ...
                for i in range(len(abbrev)):
                    ...
                    original += keyword
                    if len(original.split(" ")) >= len(abbrev):
                        break
                ...
                if test == abbrev.lower():
                    ...
                    abbrev_df = pd.concat([abbrev_df, pd.DataFrame([{'abbrev': abbrev,
                                                                     'original': original}])], ignore_index=1)
                    # abbrev_df.append({'abbrev' : abbrev,
                    # 'original' : original}, ignore_index = 1)
...
def get_abbrev_dict(texts, minimum_cnt):
    ...
    for abbrev in abbrev_list:
        original_list = abbrev_df.loc[abbrev_df['abbrev']
                                      == abbrev, 'original'].tolist()
...
def get_word_list(nlp_list):
    ...
    word_lists = [[token for token in i if len(
        token) >= 3] for i in word_lists]

    # c = Counter([x for xs in word_lists for x in set(xs)])

    word_lists_ = [[word for word in inner_list if word.casefold() not in stopwords]
                   for inner_list in word_lists]
    ...
    word_lists_ = [[word for word in inner_list if word.casefold() not in stopwords_]
                   for inner_list in word_lists_]
...
class LDA_gensim:
    ...
    def tunning_passes(self, method=['diversity', 'coherence']):
        ...
        for passes in range(2, 11, 2):
            ...
            scores_df = pd.concat(
                [scores_df, pd.DataFrame([score_dict])], ignore_index=1)
                ...
                for key in score_dict.keys():
                    if key != 'iter':
                        ...
                        growth_rate_list.append(
                            (abs(score_now)-abs(score_before)) / abs(score_before))
    ...
    def get_docByTopics(self):
        ...
        for corp in corpus:
            ...
            docByTopics = pd.concat(
                [docByTopics, pd.DataFrame([DICT])], ignore_index=1)
    ...
    def get_wordByTopics(self):
        ...
        for i in range(0, model.num_topics):
            ...
            topic_word_df = pd.concat(
                [topic_word_df, pd.DataFrame([DICT])], ignore_index=1)
    ...
    def get_TVByTime(self,
                     data,
                     time_col):

        model = self.model_final,
        self.get_docByTopics()
        topic_doc_df = self.docByTopics

        topic_doc_df = pd.DataFrame(topic_doc_df)
        topic_doc_df['time'] = data[time_col]
        ...
        for col in range(0, self.k):
            ...
            topic_time_df = pd.concat(
                [topic_time_df, pd.DataFrame([DICT])], ignore_index=1)
        ...
        self.topic_time_df = topic_time_df

        return (self.topic_time_df)

    def get_summary_TVByTime(self, recent_period):

        topic_time_df = self.topic_time_df
        ...
        self.topic_time_df_summary = result[[
            'topic', 'volume', 'volume_recent', 'CAGR', 'CAGR_recent']]

        return (self.topic_time_df_summary)
    ...
    def get_most_similar_doc2topic(self, data_sample,
        ...
        for col in range(self.docByTopics.shape[1]):
            ...
            for n in range(1, top_n+1):
                ...
                result_df = pd.concat(
                    [result_df, pd.DataFrame([DICT])], ignore_index=1)
    ...
    def evaluate_model(self, model, measure_list):
        ...
        if 'diversity' in measure_list:
            ...
            for col in topwordthetaTopics_matrix.columns:
                topwordthetaTopics_matrix[col] = normalize2one(
                    topwordthetaTopics_matrix[col])
    ...
    def save(self, directory, file_name, data, recent_period=10):
        ...
        pd.DataFrame(self.get_docByTopics()).to_excel(
            writer, sheet_name='docByTopics', index=1)
        pd.DataFrame(self.get_topicProportion()).to_excel(
            writer, sheet_name='topic_proportion', index=1)
        pd.DataFrame(self.get_topwordByTopics()).to_excel(
            writer, sheet_name='topwordByTopics', index=1)
        self.get_most_similar_doc2topic(data, title='title', date='year_application').to_excel(
            writer, sheet_name='titleByTopics', index=1)
        pd.DataFrame(self.get_wordByTopics()).to_excel(
            writer, sheet_name='wordByTopics', index=1)
